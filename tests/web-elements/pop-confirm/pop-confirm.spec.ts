import { test, expect, Page, Locator } from "@playwright/test";
    ...
    test(`Verify Pop Confirm`, async ({ page }) => {
        // Go to page checkbox
        await page.goto("https://test-with-me-app.vercel.app/learning/web-elements/components/modal");
        await selectButtonOnPopConfirm('Show Confirm', 'Are you sure delete this task?', 'Yes', page)
        await expect(page.getByText('Status: OK')).toBeVisible();
    })

    async function selectButtonOnPopConfirm(modalButtonName:string, modalName:string, action:string ,page:Page) {
